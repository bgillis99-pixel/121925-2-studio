...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${isDarkMode ? 'bg-gray-900' : 'bg-navy'} font-sans text-navy dark:text-gray-100 overflow-x-hidden transition-colors duration-300`}>
      ...
      <header className="bg-navy dark:bg-gray-800 px-4 py-4 sticky top-0 z-40 border-b-2 border-white/10 pt-safe flex flex-col gap-4">
        <div className="flex justify-between items-start">
            <div className="flex flex-col cursor-pointer" onClick={() => setCurrentView(AppView.HOME)}>
                ...
                <p className="text-vibrantGreen text-xs font-black tracking-widest uppercase mt-1">Heavy Duty Support</p>
            </div>
            <button 
              onClick={handleInstallClick} 
              className="bg-white/10 border-2 border-white/20 text-white text-xs font-black px-4 py-2.5 rounded-xl uppercase tracking-wider hover:bg-white/20 transition-all shadow-lg active:scale-95"
            >
              APP
            </button>
        </div>
        ...
        <div className="flex gap-2">
            <a href="tel:6173596953" className="flex-1 btn-heavy py-3.5 rounded-xl flex items-center justify-center gap-2 text-xs !bg-white !text-navy shadow-xl border-navy active:scale-95 transition-all">
                CALL
            </a>
            <button onClick={handleShare} className="flex-1 btn-heavy py-3.5 rounded-xl flex items-center justify-center gap-2 text-xs !bg-white !text-navy shadow-xl border-navy active:scale-95 transition-all">
                SHARE
            </button>
            <button onClick={() => setCurrentView(AppView.HOME)} className="flex-1 btn-heavy py-3.5 rounded-xl flex items-center justify-center gap-2 text-xs !bg-vibrantGreen !text-navy !border-navy shadow-xl active:scale-95 transition-all">
                FIND TESTER
            </button>
        </div>
      </header>
      ...
      {/* MAIN CONTENT AREA */}
      <main className="flex-1 px-4 pt-6 pb-24 max-w-lg mx-auto w-full flex flex-col">
        ...
        {/* FOOTER INFO */}
        <div className="mt-auto py-10 text-center border-t border-white/10">
            <p className="text-xs font-black text-white/50 uppercase tracking-[0.15em] leading-relaxed px-4">
      ...
      {/* NAVIGATION BAR */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t-2 border-gray-200 dark:border-gray-700 pb-safe pt-2 px-2 flex justify-around items-end z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.15)] h-[90px] transition-colors">
        {[
            ...
            { id: AppView.ASSISTANT, icon: "M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z", label: "AI COACH" },
            ...
            <button key={btn.id} onClick={() => setCurrentView(btn.id as AppView)} className={`flex flex-col items-center justify-center flex-1 h-full pb-3 transition-all duration-300 ${currentView === btn.id ? '-translate-y-1' : 'opacity-60'}`}>
                <div className={`p-2 rounded-2xl mb-1 transition-all ${currentView === btn.id ? 'bg-navy text-white shadow-xl scale-110' : 'text-gray-500 dark:text-gray-400'}`}>
                ...
                <span className={`text-[10px] font-black tracking-tight uppercase ${currentView === btn.id ? 'text-navy dark:text-white' : 'text-gray-500 dark:text-gray-400'}`}>{btn.label}</span>
            </button>
        ))}
      </nav>
    </div>
