...
const VinChecker: React.FC<Props> = ({ onAddToHistory, onNavigateChat, onNavigateEducation, onInstallApp, onShare }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-8 animate-in fade-in duration-700 pb-16">
      ...
      {/* SECTION 1: DISPATCH FINDER - FLASHY RED */}
      <div className="bg-shine-red card-shine rounded-[2.5rem] shadow-2xl overflow-hidden border-4 border-navy p-8 space-y-6">
      ...
      {/* SECTION 2: COMPLIANCE CHECK - FLASHY RED */}
          ...
          <div className="space-y-6">
              <div className="space-y-3">
                  <div className="flex justify-between items-center px-2">
                    ...
                    <span className="text-[10px] text-white/50 font-black uppercase">Heavy Outline Required</span>
                  </div>
                  <div className="flex gap-3">
                      <input 
                        type="text" 
                      ...
                      />
                      <button 
                        onClick={() => setSearchMode(searchMode === 'VIN' ? 'OWNER' : 'VIN')} 
                        className="px-4 bg-white border-4 border-navy rounded-2xl text-xs flex flex-col items-center justify-center active:bg-navy active:text-white transition-all text-navy font-black shadow-lg"
                      >
              ...
              <div className="space-y-2">
                  ...
                  <p className="text-white text-center text-xs font-bold uppercase tracking-[0.2em] opacity-60">Search Official State Records</p>
              </div>
          </div>
      </div>

      {/* SECTION 3: AI COACH - FLASHY NAVY */}
      <div className="bg-navy p-8 rounded-[2.5rem] shadow-2xl border-4 border-white/10 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-48 h-48 bg-electricNavy blur-[80px] opacity-40 group-hover:opacity-60 transition-opacity"></div>
      ...
      {countyResult && (
          <div className="fixed inset-0 z-[100] bg-navy/90 backdrop-blur-xl flex items-center justify-center p-4 overflow-y-auto" onClick={() => setCountyResult(null)}>
              <div className="bg-white dark:bg-gray-800 rounded-[3rem] p-8 w-full max-w-sm shadow-2xl space-y-8 text-center border-t-[16px] border-teslaRed transition-all animate-in zoom-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex gap-4">
                            {['OVI', 'OBD'].map((type) => (
                                <button 
                                    ...
                                    className={`flex-1 py-6 rounded-2xl font-black text-base transition-all border-4 ${testType === type ? 'bg-navy text-white border-navy shadow-xl scale-105' : 'bg-white text-navy border-gray-200'}`}
                                >
                                    {type}
                                </button>
                            ))}
                      ...
                      <div className="grid grid-cols-2 gap-4 text-left">
                          <div className="space-y-2">
                            ...
                            <input 
                                ...
                                className="w-full p-4 rounded-xl bg-white border-2 border-gray-200 text-lg font-black outline-none focus:border-navy"
                            />
                          </div>
                          <div className="space-y-2">
                  ...
                  <div className="space-y-4">
                      <a href={smsLink} className="block w-full py-6 rounded-2xl shadow-[0_15px_40px_rgba(34,197,94,0.4)] text-2xl bg-vibrantGreen text-white border-4 border-navy font-black uppercase active:scale-[0.98] transition-all">
                          TEXT DISPATCH
                      </a>
                      <a href="tel:6173596953" className="block w-full py-5 rounded-2xl text-xl bg-teslaRed text-white border-4 border-navy font-black shadow-xl uppercase active:scale-[0.98] transition-all">
                          CALL: 617-359-6953
                      </a>
                  </div>
