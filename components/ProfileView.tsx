...
const ProfileView: React.FC<Props> = ({ user, history, onLogin, onRegister, onLogout, onAdminAccess, isOnline = true, isDarkMode, toggleTheme }) => {
  ...
  const SettingsCard = () => (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border-4 border-navy">
        <h3 className="font-black text-[#003366] dark:text-white text-lg mb-4 uppercase tracking-tighter">App Settings</h3>
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                {notifPermission === 'granted' ? (
                    <span className="text-green text-[10px] font-black bg-green/10 px-2 py-1 rounded">ACTIVE</span>
                ) : (
            ...
            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                <div>
                    <h4 className="font-black text-sm text-gray-700 dark:text-gray-200 uppercase leading-none">Interface Theme</h4>
                    <p className="text-[10px] text-gray-500 font-bold uppercase mt-1">
                        {isDarkMode ? 'Night Mode Active' : 'Day Mode Active'}
                    </p>
                </div>
                <button 
                    onClick={toggleTheme} 
    ...
    return (
      <div className="space-y-6 pb-20">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-xl border-4 border-navy">
            <h2 className="text-2xl font-black text-[#003366] dark:text-white mb-6 text-center uppercase tracking-tighter">{isRegistering ? 'Create Account' : 'Guest Mode'}</h2>
            ...
            <div className="mt-4 text-center">
                <button onClick={() => setIsRegistering(!isRegistering)} className="text-[#003366] dark:text-blue-400 text-[10px] font-black hover:underline uppercase tracking-widest">
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20 space-y-6">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border-4 border-navy flex justify-between items-center">
        <div>
            <h2 className="text-xl font-black text-[#003366] dark:text-white truncate max-w-[200px] uppercase tracking-tighter">{user.email}</h2>
        ...
        <button onClick={onLogout} className="text-red-600 dark:text-red-400 text-xs font-black border-2 border-red-600 dark:border-red-400 px-4 py-2 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 uppercase">LOGOUT</button>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border-4 border-navy overflow-hidden">
            <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border-b-2 border-navy">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-black text-[#003366] dark:text-white uppercase tracking-tighter">Recent Checks</h3>
